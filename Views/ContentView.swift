                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        if store.cases.isEmpty {
                            // Empty state — real data import instructions
                            VStack(spacing: 20) {
                                ...
                                Text("Import real cloud sync logs\nto start your investigation")
                                    .font(.system(size: 14))
                                    .foregroundColor(Color.white.opacity(0.25))
                                    .multilineTextAlignment(.center)

                                // How to get logs guide
                                VStack(alignment: .leading, spacing: 12) {
                                    Text("HOW TO GET YOUR LOGS")
                                        .font(.system(size: 10, weight: .black))
                                        .foregroundColor(Color(hex: "#00D4FF").opacity(0.4))
                                        .tracking(1.5)

                                    LogSourceRow(provider: .iCloud,
                                                 instruction: "Console.app → search 'CloudDocs' → export")
                                    LogSourceRow(provider: .googleDrive,
                                                 instruction: "Admin Console → Reports → Drive → Export CSV")
                                    LogSourceRow(provider: .dropbox,
                                                 instruction: "Admin Console → Activity → Export to CSV")
                                    LogSourceRow(provider: .oneDrive,
                                                 instruction: "M365 Compliance Center → Audit → Export")
                                }
                                .padding(16)
                                .background(RoundedRectangle(cornerRadius: 14)
                                    .fill(Color.white.opacity(0.03))
                                    .overlay(RoundedRectangle(cornerRadius: 14)
                                        .strokeBorder(Color(hex: "#00D4FF").opacity(0.08), lineWidth: 0.5)))
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        // Auto-detect provider hint
                        if !logText.isEmpty {
                            let detected = LogParser().autoDetectProvider(from: logText)
                            if detected != .unknown && detected != selectedProvider {
                                HStack(spacing: 8) {
                                    Image(systemName: "wand.and.stars")
                                        .font(.system(size: 12))
                                        .foregroundColor(Color(hex: "#00D4FF"))
                                    Text("Detected: \(detected.rawValue) — tap to switch")
                                        .font(.system(size: 12))
                                        .foregroundColor(Color(hex: "#00D4FF").opacity(0.8))
                                    Spacer()
                                    Button("Switch") { selectedProvider = detected }
                        ...
                        // Analyze button — requires real log data
                        Button(action: {
                            guard !logText.isEmpty else { return }
                            store.analyzeText(logText, provider: selectedProvider, caseName: caseName)
                            dismiss()
                        }) {
                            Text("Analyze Logs")
                                ...
                                .background(RoundedRectangle(cornerRadius: 14)
                                    .fill(LinearGradient(
                                        colors: logText.isEmpty
                                            ? [Color.gray, Color.gray]
                                            : [Color(hex: "#00D4FF"), Color(hex: "#0052D4")],
                                        startPoint: .leading, endPoint: .trailing
                                    )))
                        }
